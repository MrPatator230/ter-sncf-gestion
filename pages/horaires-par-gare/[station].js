...
export default function StationSchedule() {
  ...
  const getAllStations = (schedule) => {
    ...
    stations.push({
      ...
      track: trackAssignments[schedule.id]?.[schedule.departureStation] || '-',
      status
    });
    ...
    if (schedule.servedStations) {
      schedule.servedStations.forEach(station => {
        ...
        stations.push({
          ...
          track: trackAssignments[schedule.id]?.[stationName] || '-',
          status
        });
      });
    }
    ...
    stations.push({
      ...
      track: trackAssignments[schedule.id]?.[schedule.arrivalStation] || '-',
      status
    });
